				     ...
				     struct cxlmi_cmd_get_event_records *ret)
{
	...
	struct cxlmi_cmd_get_event_records_request *req_pl;
	_cleanup_free_ struct cxlmi_cci_msg *req = NULL;
	_cleanup_free_ struct cxlmi_cci_msg *rsp = NULL;
	...
	req_pl = (struct cxlmi_cmd_get_event_records_request *)req->payload;
	 ...
	 */
	rsp_sz = sizeof(*rsp) + (CXLMI_MAX_SUPPORTED_LOGS * sizeof(*rsp_pl->records));
	rsp = calloc(1, rsp_sz);
	if (!rsp)
		return -1;
