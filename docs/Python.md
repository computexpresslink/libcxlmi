...
- Python 3.6 or later
- clang2py (from ctypeslib2)
- LLVM clang + clang python package with the *same version* as LLVM clang
...
## Troubleshooting
If you are using Python 3.12+: the ctpyes.CDLL._filepath attribute was deprecated,
breaking ctypeslib2 as of Novemeber 2025. This causes the following error:
...
This is a limitation of ctypes, which parses the clang AST to generate the bindings.
...
clang2py/ctypes cannot create a Python attribute with a dynamic-length array, so the generated bindings declare
